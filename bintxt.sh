...
def reformat_txt(base, txt_path, old_state, new_cfg, val_cfg, run_dir, log):
    ...
    Extracts raw bytes using old settings, re-serializes with new settings + new labels.
    Backs up the old .txt to run_dir/rollback/ before overwriting.
    Returns True on success."""

    def _state_to_cfg(s):
        """Reconstruct a minimal bin_cfg dict from a saved state fingerprint."""
        return {
            ...
            'labels':             [],   # labels don't affect byte extraction
            'file':               f'{base}.bin',
        }
    ...
    # ── Re-serialize raw bytes → new .txt ────────────────────────────────────
    ...
    for base in all_bases:
        ...
        if no_yaml_entry:
            ...
            if cfg_changed and has_txt and prev_state:
                # Reformat .txt from old settings → new settings, entirely from the .txt
        ...
        if has_bin:
            ...
            if unpacked_txt is not None:
                ...
                # Move .bin out of configs/ — it has been unpacked, configs/ is for .txt only
                moved_bin = run_dir / f'{base}.bin'
                shutil.move(str(bin_path), str(moved_bin))
                bin_path = moved_bin  # update ref for verify step
